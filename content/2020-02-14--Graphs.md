...
**1.** We'll have an array called components where `components[vertex]` (we'll initialize this array to 0 in the beginning) will be the component number for that vertex. For example, in the image above, let's say
    - `0 1 2 3` are in component 0
    - `4 5 6 7` are in component 1
    - `8 9 10` are in component 2. 
    
 Therefore, our array would look like this: 
 ```
...
- Clear out the `visited` array so that all entries are false. 
...
```
`count` is the variable that'll allow us to keep track of the component number we're on. Once the above loop ends, our `components` array would look like this:
...
**3.** We'll call DFS again but this time with the vertex number that has the value 0 in the components array. We'll repeat the process until there are no more 0s in the components array.
