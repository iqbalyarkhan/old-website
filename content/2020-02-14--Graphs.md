...
The function goes to the `adjList[1]` and pushes back 4 and conversely, goes to `adjList[4]` and pushes a 1. Since our graph is undirected, it means that if 1 is connected to 4, then 4 is also connected to 1. Once the connection is made, our vectors would look like this:
...
The process repeats until no more edges are to be added. We do not have a check in our `addEdge()` function to see if an edge already exists because we assume that the client will always pass in distinct edges.
