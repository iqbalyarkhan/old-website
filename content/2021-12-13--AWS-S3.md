...
- **Bucket Policies**:
  S3 bucket policies that specify what action are allowed or denied (example user A can PUT but not DELETE). Bucket policies are obviously bucket wide. These are IAM policies for S3 buckets. This is more general as compared to ACLs. Bucket policies are resource policies. Resource policies control what entities can access this particular resource.In contrast, identity policies control what resources an identity can access.
